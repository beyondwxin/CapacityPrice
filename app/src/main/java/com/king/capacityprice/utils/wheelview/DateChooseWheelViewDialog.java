package com.king.capacityprice.utils.wheelview;
...
import com.king.capacityprice.utils.wheelview.adapters.AbstractWheelTextAdapter;
...
//  mDateCalendarTextView.setText(" " + arry_date.get(wheel.getCurrentItem()));
    ...
    private void initDate() {
        ...
        mDateWheelView.setVisibleItems(3);
        ...
        switch (v.getId()) {
            case R.id.sure_btn://确定选择按钮监听
                ...
                if (mBlnTimePickerGone) {
                    ...
                    try {
                        Date currentTime = dateFormat.parse(year + "-" + month + "-" + day);
                        Date chooseTime = dateFormat.parse(strTimeToDateFormat(mYearStr, mDateStr));
                        LogUtil.e("df", currentTime + "  " + chooseTime);
                        if (compareDate(chooseTime, currentTime)) {
